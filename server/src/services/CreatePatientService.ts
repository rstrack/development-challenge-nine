...
export class CreatePatientService {
  ...
  async execute({
    ...
    const patient = new Patient({ birthDate: dateBirthDate, email, name })
    const patientId = patient.id

    const [newPatient, newAddress] = await transaction([
      this.patientRepository.save(patient),
      this.addressRepository.save(new Address({ ...address, patientId })),
    ])
